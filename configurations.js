function initializeCircle(num_boids) {
  for (let i = 0; i < num_boids; i++) {
    let start_vector = p5.Vector.fromAngle(TWO_PI*i/num_boids).mult(width/2.1);
    let goal_vector = p5.Vector.fromAngle(TWO_PI*i/num_boids).mult(-width/2.1);

    boids[i] = new Boid(start_vector, goal_vector, color(255*i/num_boids, 255, 255));
  }
}

function initializeStriaghtOnPair(num_boids) {
  initializeCircle(2);
}
